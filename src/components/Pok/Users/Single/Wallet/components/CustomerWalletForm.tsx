...
import { useCustomerWalletForm } from './useCustomerWalletForm';

type CustomerWalletFormProps = {
  id: string;
  points: number;
  handleClose: () => void;
  handleRefresh: () => void;
...
export const CustomerWalletForm = ({
  id,
  points = 0,
  handleClose,
  handleRefresh,
}: CustomerWalletFormProps) => {
  const { form, error, isLoading, t } = useCustomerWalletForm({
    id,
    handleClose,
    handleRefresh,
  });
  return (
    ...
    >
      ...
      <div>
        <Form {...{ form }}>
          <div className='items-center gap-10 md:grid md:grid-cols-2'>
